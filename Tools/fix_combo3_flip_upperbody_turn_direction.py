...
REPORT_PATH = r"E:\UEProj\Sekiro\SekiroDemo\Saved\Codex\combo3_flip_upperbody_turn_direction_report.json"
TARGET_COMPONENT_DELTA_DEGREES = -296.647
...
def q_from_yaw(degrees):
    radians = math.radians(degrees)
    return unreal.Quat(0.0, math.sin(radians * 0.5), 0.0, math.cos(radians * 0.5))
...
def summarize(anim):
    ...
    samplers = {
        ...
        "PelvisLocal": lambda frame: lib.get_bone_pose_for_frame(anim, "Pelvis", frame, False).rotation,
        "RootRotYLocal": lambda frame: lib.get_bone_pose_for_frame(anim, "RootRotY", frame, False).rotation,
        "RootPosPelvisComponent": lambda frame: q_mul(
...
def main():
    report = {
        "asset": ASSET_PATH,
        "status": "running",
        "target_component_delta_degrees": TARGET_COMPONENT_DELTA_DEGREES,
    }
    try:
        if os.path.exists(DISK_PATH):
            backup = DISK_PATH + ".bak_pre_flip_upperbody_turn_direction_" + time.strftime("%Y%m%d_%H%M%S")
        ...
        first_rootpos = lib.get_bone_pose_for_frame(anim, "RootPos", 0, False).rotation
        first_rootroty = lib.get_bone_pose_for_frame(anim, "RootRotY", 0, False).rotation
        start_component = q_mul(first_rootpos, first_rootroty)

        positions = []
        rotations = []
        scales = []
        for frame in range(frames):
            alpha = frame / float(frames - 1)
            desired_component = q_mul(q_from_yaw(TARGET_COMPONENT_DELTA_DEGREES * smoothstep(alpha)), start_component)
            ...
            rotations.append(q_mul(q_inverse(rootpos.rotation), desired_component))
