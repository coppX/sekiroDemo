...
REPORT_PATH = r"E:\UEProj\Sekiro\SekiroDemo\Saved\Codex\combo3_rootroty_300_visual_turn_report.json"
TARGET_TURN_DEGREES = -300.0
...
def summarize(anim):
    ...
    for name, sampler in {
        ...
        yaws = [q_yaw_degrees(sampler(frame)) for frame in range(frames)]
...
def main():
    report = {"asset": ASSET_PATH, "status": "running", "target_turn_degrees": TARGET_TURN_DEGREES}
    try:
        if os.path.exists(DISK_PATH):
            backup = DISK_PATH + ".bak_pre_rootroty_300_visual_turn_" + time.strftime("%Y%m%d_%H%M%S")
        ...
        desired_start_component = q_mul(first_rootpos, first_rootroty)
        ...
        for frame in range(frames):
            ...
            turn = q_from_yaw(TARGET_TURN_DEGREES * smoothstep(alpha))
            desired_component = q_mul(turn, desired_start_component)
