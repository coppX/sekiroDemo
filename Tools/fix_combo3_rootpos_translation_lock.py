...
REPORT_PATH = r"E:\UEProj\Sekiro\SekiroDemo\Saved\Codex\combo3_rootpos_translation_lock_report.json"
...
def main():
    ...
    try:
        if os.path.exists(DISK_PATH):
            backup_path = DISK_PATH + ".bak_pre_rootpos_translation_lock_" + time.strftime("%Y%m%d_%H%M%S")
        ...
        frames = max(2, int(round(anim.get_play_length() * 30)) + 1)
        rootpos_first = lib.get_bone_pose_for_frame(anim, "RootPos", 0, False)
        positions = [rootpos_first.translation for _ in range(frames)]
        rotations = []
        scales = []
        for frame in range(frames):
            current = lib.get_bone_pose_for_frame(anim, "RootPos", frame, False)
            rotations.append(current.rotation)
            scales.append(current.scale3d)

        report["set_upper_ok"] = anim.controller.set_bone_track_keys("RootPos", positions, rotations, scales, True)
        report["set_lower_ok"] = anim.controller.set_bone_track_keys("rootpos", positions, rotations, scales, True)
