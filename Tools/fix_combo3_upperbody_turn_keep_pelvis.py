...
RESTORE_PATH = r"E:\UEProj\Sekiro\SekiroDemo\Content\Animation\Sekiro\C0000\GroundAttack_SM\a050_300020.uasset.bak_pre_rootroty_compensate_20260602_142308"
REPORT_PATH = r"E:\UEProj\Sekiro\SekiroDemo\Saved\Codex\combo3_upperbody_turn_keep_pelvis_report.json"
...
def main():
    report = {"asset": ASSET_PATH, "status": "running", "restore_path": RESTORE_PATH}
    try:
        if not os.path.exists(RESTORE_PATH):
            raise FileNotFoundError(RESTORE_PATH)
        if os.path.exists(DISK_PATH):
            backup = DISK_PATH + ".bak_pre_upperbody_turn_keep_pelvis_" + time.strftime("%Y%m%d_%H%M%S")
            shutil.copy2(DISK_PATH, backup)
            report["backup"] = backup
        shutil.copy2(RESTORE_PATH, DISK_PATH)
        report["restored_first"] = True
