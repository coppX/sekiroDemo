...
void FMonolithAnimationActions::RegisterActions(FMonolithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("animation"), TEXT("set_transition_rule"),
		...
		FParamSchemaBuilder()
			...
			.Build());
	Registry.RegisterAction(TEXT("animation"), TEXT("set_state_machine_entry"),
		TEXT("Connect a state machine entry node to the requested initial state"),
		FMonolithActionHandler::CreateStatic(&HandleSetStateMachineEntry),
		FParamSchemaBuilder()
			.Required(TEXT("asset_path"), TEXT("string"), TEXT("Animation Blueprint asset path"))
			.Required(TEXT("machine_name"), TEXT("string"), TEXT("State machine name"))
			.Required(TEXT("state_name"), TEXT("string"), TEXT("Initial state name"))
			.Build());
...
FMonolithActionResult FMonolithAnimationActions::HandleSetBoneTrackKeys(const TSharedPtr<FJsonObject>& Params)
{
	...
	Controller.OpenBracket(FText::FromString(TEXT("Set Bone Track Keys")), false);
	Controller.AddBoneCurve(BoneFName, false);
	Controller.SetBoneTrackKeys(BoneFName, Positions, Rotations, Scales, false);
...
FMonolithActionResult FMonolithAnimationActions::HandleSetStateMachineEntry(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath = Params->GetStringField(TEXT("asset_path"));
	FString MachineName = Params->GetStringField(TEXT("machine_name"));
	FString StateName = Params->GetStringField(TEXT("state_name"));
	...
	if (!SMGraph->EntryNode) return FMonolithActionResult::Error(FString::Printf(TEXT("State machine '%s' has no entry node"), *MachineName));
	...
	if (!EntryOutputPin) return FMonolithActionResult::Error(TEXT("Entry node has no output pin"));
	if (!StateInputPin) return FMonolithActionResult::Error(FString::Printf(TEXT("State '%s' has no input pin"), *StateName));

	if (EntryOutputPin->LinkedTo.Contains(StateInputPin))
	{
		TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
		Root->SetStringField(TEXT("asset_path"), AssetPath);
		Root->SetStringField(TEXT("machine_name"), MachineName);
		Root->SetStringField(TEXT("entry_state"), StateName);
		Root->SetBoolField(TEXT("already_connected"), true);
		return FMonolithActionResult::Success(Root);
	}

	GEditor->BeginTransaction(FText::FromString(TEXT("Set State Machine Entry")));
	SMGraph->Modify();
	...
	const bool bConnected = Schema->TryCreateConnection(EntryOutputPin, StateInputPin);
	GEditor->EndTransaction();

	if (!bConnected)
	{
		return FMonolithActionResult::Error(FString::Printf(TEXT("TryCreateConnection failed for entry -> '%s'"), *StateName));
	}
